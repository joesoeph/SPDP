...
class ProofSpend_model extends CI_Model
{

    public $table = 'trnproofspend';
    public $sequence = 'seqproofspend';
    public $id = 'ProofSpendId';
    public $order = 'ASC';
    ...
    function lastNoUrutReimburse()
		{
      $sql = "SELECT MAX(NoUrutReimburse) as NoUrutReimburse FROM $this->table";
      return $this->db->query($sql)->result();
    }
    ...
    function getDetail($id = NULL)
      ...
      $sql = "SELECT a.*, 
										 ...
										 j.name as RecipientName, 
										 k.JabatanName as RecipientJabatan
              FROM $this->table a 
              ...
              LEFT JOIN users j ON j.id =  a.Recipient
              LEFT JOIN mstjabatan k ON k.JabatanId = j.JabatanId
              WHERE a.ProofSpendId = '$id'";
    ...
    function recipientSelected($id = NULL)
      ...
      $sql = "SELECT a.id, a.name, c.JabatanName, CASE WHEN (b.Recipient IS NOT NULL) THEN \"selected = 'selected'\" ELSE 'null' END AS selected
              ...
              FROM $this->table WHERE $this->id = '$id') b ON a.id = b.Recipient
              LEFT JOIN mstjabatan c ON a.JabatanId=c.JabatanId
              WHERE a.DeletedDate IS NULL AND a.activation='Active'";
