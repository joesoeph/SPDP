$(function () {
    ...
    $('#fileupload_9').fileupload({
        ...
        data.context = $('<div/>').appendTo('#files_9');
        $.each(data.files, function (index, file) {
            var node = $('<p/>').append($('<span/>'));
            if (!index) {
                node.append(uploadButton_9.clone(true).data(data));
            }
            node.appendTo(data.context);
        });
    }).on('fileuploadprocessalways', function (e, data) {
        ...
        $.each(data.result.files, function (index, file) {
            if (file.url) {
                $("#filename_9").val(file.name);
                $("#fileupload_9").attr('disabled', true);
                $("#btnCancel_9").removeAttr('style');
                $("#btnCancel_9").on("click", function() {
                    $("#filename_9").val('');
                    $("#fileupload_9").attr('disabled', false);
                    $(this).hide();
                    $('#progress_9 #progress-bar_9').css('width','0%');
                });
            } else if (file.error) {
                var error = $('<span class="text-danger"/>').text(file.error);
                $(data.context.children()[index])
    ...
    $("#btnVerifikasiSubmit").on("click", function() {
        var KWITANSI = $("#filename_1").val(),
            BLA = $("#filename_2").val(),
            BAPALAT = $("#filename_3").val(),
            REKAPAN = $("#filename_4").val(),
            TIMESHEET = $("#filename_5").val(),
            SPSA = $("#filename_6").val(),
            FAKTURPAJAK = $("#filename_7").val(),
            SPTMASA = $("#filename_8").val(),
            JURNALPPN = $("#filename_9").val();

        if (KWITANSI!='' || BLA!='' || BAPALAT!='' || REKAPAN!='' || TIMESHEET!='' || SPSA!='' || FAKTURPAJAK!='' || SPTMASA!='' || JURNALPPN!='') {
            $.ajax({
